<template>
  <div class="item3">
    <div v-for="item in data" :key="item.id" class="item">
      <div class="ban">已被{{item.num}}人关注</div>
      ...
      <a href="/" class="img">
        ...
        <div class="content">
          ...
          <p>
           ...
           <div class="icon">
            <van-icon name="chat-o">{{item.commentCount}}</van-icon>
            <van-icon name="star">{{item.averageScore}}</van-icon>
...
.ban {
  ...
  color: #fff;
  line-height: 25px;
  background-image: linear-gradient(133deg, #e5a9ff, #8e21fc);
